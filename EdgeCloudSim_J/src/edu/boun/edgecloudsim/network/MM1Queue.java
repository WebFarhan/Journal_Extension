...
public class MM1Queue extends NetworkModel {
    ...
    */
	...
	public double getUploadDelay(int sourceDeviceId, int destDeviceId, double dataSize) {
		...
		if(destDeviceId == SimSettings.CLOUD_DATACENTER_ID){
			double wlanDelay = getWlanUploadDelayCloud(accessPointLocation, CloudSim.clock(),SimSettings.getInstance().getWanPropogationDelay());
	...
	public double getTransferDelay(int sourceDeviceId, int destDeviceId, double dataSize) {
		double delay = 0;
    ...
    */
	...
	private double getWlanUploadDelayCloud(Location accessPointLocation, double time, double propDelay) {
		return calculateMM1(propDelay,
				SimSettings.getInstance().getWlanBandwidth(),
				WlanPoissonMean,
				avgTaskInputSize,
				getDeviceCount(accessPointLocation, time));
